...
@export var rendevouz_address = "192.168.1.56" 
...
func _process(delta):
	...
	if server_udp.get_available_packet_count() > 0:
		print("In holePunch node packet received from udp_server")
		...
		if not recieved_peer_info:
			...
			if packet_string.begins_with(SERVER_INFO):
				...
				'''packet_string = packet_string.right(- 6) #kendu ezkerrean dagoen peers:
...
func _ping_peer():
	...
	if not recieved_peer_confirm and greets_sent < response_window:
		for p in peer.keys():
			...
			print("peer: " + str(peer[p].address))
			print("port: " + str(peer[p].port))
